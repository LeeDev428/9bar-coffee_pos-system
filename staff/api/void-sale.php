...
// Check if user is logged in (both admin and staff allowed)
if (!$auth->isLoggedIn()) {
...
$staffPassword = $input['staff_password'] ?? '';
...
if (empty($staffPassword)) {
    echo json_encode(['success' => false, 'error' => 'Staff password is required']);
    exit;
}

try {
    // Verify staff password (support multiple password formats like login)
    ...
    $passwordValid = false;
    if (password_verify($staffPassword, $userCheck['password'])) {
        // Bcrypt hash
        $passwordValid = true;
    } else if (md5($staffPassword) === $userCheck['password']) {
        // MD5 hash (used by seeder)
        $passwordValid = true;
    } else if ($userCheck['password'] === $staffPassword) {
        // Plain text password (development)
        $passwordValid = true;
    } else if (($userCheck['username'] === 'admin' && $staffPassword === 'admin123') || 
              ($userCheck['username'] === 'staff1' && $staffPassword === 'staff123')) {
