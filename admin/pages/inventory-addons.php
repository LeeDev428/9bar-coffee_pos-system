<?php
// Admin Inventory - Add-Ons Management
$page_title = 'INVENTORY - ADD-ONS';
include '../components/main-layout.php';

// Handle delete request
if (isset($_GET['delete_addon'])) {
    try {
        $addonId = intval($_GET['delete_addon']);
        $db->query("DELETE FROM addons WHERE addon_id = ?", [$addonId]);
        showAlert('Add-on deleted successfully!', 'success');
        header('Location: inventory-addons.php');
        exit;
    } catch (Exception $e) {
        showAlert('Error deleting add-on: ' . $e->getMessage(), 'error');
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'add_addon':
                try {
                    $addonName = sanitizeInput($_POST['addon_name']);
                    ...
                    $db->query("INSERT INTO addons (addon_name, unit, current_stock, reorder_level, cost_per_unit, supplier, status) VALUES (?, ?, ?, ?, ?, ?, 'active')", [
                        $addonName, $unit, $currentStock, $reorderLevel, $costPerUnit, $supplier
                    ]);
                    
                    showAlert('Add-on added successfully!', 'success');
                } catch (Exception $e) {
                    showAlert('Error adding add-on: ' . $e->getMessage(), 'error');
                }
                break;
                
            case 'update_addon':
                try {
                    $addonId = intval($_POST['addon_id']);
                    $addonName = sanitizeInput($_POST['addon_name']);
                    $unit = sanitizeInput($_POST['unit']);
                    $currentStock = floatval($_POST['current_stock']);
                    $reorderLevel = floatval($_POST['reorder_level']);
                    ...
                    $db->query("UPDATE addons SET addon_name = ?, unit = ?, current_stock = ?, reorder_level = ?, cost_per_unit = ?, supplier = ?, status = ? WHERE addon_id = ?", [
                        $addonName, $unit, $currentStock, $reorderLevel, $costPerUnit, $supplier, $status, $addonId
                    ]);
                    
                    showAlert('Add-on updated successfully!', 'success');
                } catch (Exception $e) {
                    showAlert('Error updating add-on: ' . $e->getMessage(), 'error');
                }
                break;
        }
    }
...
// Get add-ons data
$addons = $db->fetchAll("SELECT * FROM addons ORDER BY addon_name");

// Calculate stats
$totalAddons = count($addons);
$lowStockCount = count(array_filter($addons, function($item) { return $item['current_stock'] <= $item['reorder_level']; }));
$totalValue = array_sum(array_map(function($item) { return $item['current_stock'] * $item['cost_per_unit']; }, $addons));
?>
...
<div class="inventory-header">
    <div>
        <h2 style="margin: 0; color: #2c3e50;">Inventory Management - Add-Ons</h2>
        <p style="color: #7f8c8d; margin: 5px 0 0 0;">Manage product add-ons and extras</p>
    </div>
...
<div class="stats-grid">
    <div class="stat-card">
        <div class="stat-value" style="color: #3498db;"><?php echo $totalAddons; ?></div>
...
<div class="content-card">
    <div class="card-header">
        <h3 style="margin: 0;">Add-Ons List</h3>
        <input type="text" id="searchInput" placeholder="Search add-ons..." style="padding: 8px; border: 1px solid #ddd; border-radius: 4px;" onkeyup="filterTable()">
    </div>
        ...
        <table id="addonsTable">
            ...
            <tbody>
                <?php foreach ($addons as $addon): 
                    $isLowStock = $addon['current_stock'] <= $addon['reorder_level'];
                ?>
                <tr>
                    <td><strong><?php echo htmlspecialchars($addon['addon_name']); ?></strong></td>
                    <td><?php echo htmlspecialchars($addon['unit']); ?></td>
                    <td>
                        <strong style="color: <?php echo $isLowStock ? '#e74c3c' : '#27ae60'; ?>">
                            <?php echo number_format($addon['current_stock'], 2); ?>
                        </strong>
                    </td>
                    <td><?php echo number_format($addon['reorder_level'], 2); ?></td>
                    <td>₱<?php echo number_format($addon['cost_per_unit'], 2); ?></td>
                    <td><?php echo htmlspecialchars($addon['supplier'] ?: 'N/A'); ?></td>
                    <td>
                        <span class="status-badge status-<?php echo $addon['status']; ?>">
                            <?php echo strtoupper($addon['status']); ?>
                        </span>
                        <?php if ($isLowStock): ?>
                    ...
                    <td>
                        <button class="btn btn-primary btn-sm" onclick='editAddon(<?php echo json_encode($addon); ?>)' title="Edit">
...
<div id="addonModal" class="modal">
    <div class="modal-content">
        ...
        <div class="modal-body">
            <form method="POST">
                <input type="hidden" name="action" id="formAction" value="add_addon">
                <input type="hidden" name="addon_id" id="addonId">
                
                <div class="form-grid">
                    <div class="form-group full-width">
                        <label class="form-label">Add-On Name *</label>
                        <input type="text" name="addon_name" id="addonName" class="form-control" placeholder="e.g., Extra Shot, Whipped Cream, Caramel Drizzle" required>
                    </div>
...
<div id="manageModal" class="modal">
    <div class="modal-content" style="max-width: 800px;">
        ...
        <div class="modal-body">
            <input type="text" id="manageSearchInput" placeholder="Search add-ons..." style="width: 100%; padding: 8px; margin-bottom: 15px; border: 1px solid #ddd; border-radius: 4px;" onkeyup="filterManageTable()">
                ...
                <table style="width: 100%;">
                    ...
                    <tbody id="manageTableBody">
                        ...
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($addon['unit']); ?></td>
                            <td><?php echo number_format($addon['current_stock'], 2); ?></td>
                            <td>
                                <span class="status-badge status-<?php echo $addon['status']; ?>">
                                    <?php echo strtoupper($addon['status']); ?>
                                </span>
                            ...
                            <td style="text-align: center;">
                                <button class="btn btn-danger btn-sm" onclick="deleteAddon(<?php echo $addon['addon_id']; ?>, '<?php echo addslashes($addon['addon_name']); ?>')">
...
function deleteAddon(id, name) {
    if (confirm(`Are you sure you want to delete "${name}"? This action cannot be undone.`)) {
        window.location.href = '?delete_addon=' + id;
    }
}

function openAddModal() {
    document.getElementById('modalTitle').textContent = 'Add Add-On';
    document.getElementById('formAction').value = 'add_addon';
...
function editAddon(addon) {
    ...
    document.getElementById('formAction').value = 'update_addon';
    document.getElementById('addonId').value = addon.addon_id;
    document.getElementById('addonName').value = addon.addon_name;
    document.getElementById('unit').value = addon.unit;
    document.getElementById('currentStock').value = addon.current_stock;
    document.getElementById('reorderLevel').value = addon.reorder_level;
    document.getElementById('costPerUnit').value = addon.cost_per_unit;
    document.getElementById('supplier').value = addon.supplier || '';
...
window.onclick = function(event) {
    ...
    if (event.target === addonModal) addonModal.style.display = 'none';
