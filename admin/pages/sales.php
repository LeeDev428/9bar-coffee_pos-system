...
$searchSql = '';
if ($search) {
    $searchSql = "AND (p.product_name LIKE ? OR s.sale_id LIKE ? OR u.username LIKE ? OR s.payment_method LIKE ? OR DATE_FORMAT(s.sale_date, '%Y-%m-%d') LIKE ?)";
    $searchTerm = '%' . $search . '%';
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    $exportSql = "SELECT s.sale_id, s.sale_date, s.total_amount, s.payment_method, u.username,
            ...
            (SELECT GROUP_CONCAT(CONCAT(p2.product_name, ' (', si2.quantity, ')') SEPARATOR ', ') 
             FROM sale_items si2 
             JOIN products p2 ON si2.product_id = p2.product_id 
             WHERE si2.sale_id = s.sale_id) as products_list
     FROM sales s
    ...
    fputcsv($out, ['Sale ID', 'Date', 'Products', 'Items Count', 'Payment', 'Amount']);
    foreach ($exportRows as $t) {
        fputcsv($out, [$t['sale_id'], $t['sale_date'], $t['products_list'], $t['item_count'], $t['payment_method'], $t['total_amount']]);
    }
    fclose($out);
    exit();
}
...
$querySql = "SELECT s.sale_id, s.sale_date, s.total_amount, s.payment_method, s.payment_status, u.username,
            (SELECT COUNT(*) FROM sale_items si2 WHERE si2.sale_id = s.sale_id) as item_count,
            (SELECT GROUP_CONCAT(CONCAT(p2.product_name, ' (', si2.quantity, ')') SEPARATOR ', ') 
             FROM sale_items si2 
             JOIN products p2 ON si2.product_id = p2.product_id 
    ...
    <div style="background:white; border-radius:12px; padding:30px; max-width:500px; width:90%; box-shadow:0 10px 40px rgba(0,0,0,0.3);">
        ...
        <div style="margin-bottom:15px;">
            <label style="display:block; margin-bottom:8px; font-weight:600; color:#2c3e50;">
                <i class="fas fa-lock"></i> Admin Password <span style="color:#e74c3c;">*</span>
            </label>
            <input type="password" id="adminPassword" placeholder="Enter your admin password" style="width:100%; padding:12px; border:2px solid #dee2e6; border-radius:6px; font-size:14px;" required>
        </div>
